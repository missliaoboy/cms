...
class content_model extends model {
	...
	public function add_content($data,$isimport = 0) {
		...
		$this->hits_db->insert(array('hitsid'=>$hitsid,'modelid'=>$modelid,'id'=>$id,'catid'=>$systeminfo['catid'],'updatetime'=>SYS_TIME));
		if($data['status']==99) {
			//更新到全站搜索
		...
		if($id && isset($_POST['othor_catid']) && is_array($_POST['othor_catid'])) {
			...
			//$html->show($urls[1],$urls['data']);
				...
				$this->hits_db->insert(array('hitsid'=>$hitsid,'modelid'=>$mid,'id'=>$newid,'catid'=>$cid,'updatetime'=>SYS_TIME));
			}
		}
		//END 发布到其他栏目
		//更新附件状态
