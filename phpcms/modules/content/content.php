...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			{	
				...
				$sort = intval($_GET['type_sort']) > 0 ? 'asc': 'desc';
				$type_name = $_GET['type_name'];
				if($type_name == 'wirtes'){
					$type_name = 'updatetime';
					$sort = ' desc ';
				}
				$this->db->set_model($modelid);
	...
	public function init2() {
		...
		if(isset($model[$modelid])){
			...
			$admin_username = param::get_cookie('admin_username');
			//查询当前的工作流
			$workflows = getcache('workflow_1','commons');
			...
			$workflow_steps = $workflows['steps'];
			$workflow_menu = '';
			$steps = isset($_GET['steps']) && $_GET['steps'] !== '' ? intval($_GET['steps']) : '';
			//工作流权限判断
			if($_SESSION['roleid']!=1 && $steps && !in_array($steps,$admin_privs)) showmessage(L('permission_to_operate'));
			$this->db->set_model($modelid);
			if($this->db->table_name==$this->db->db_tablepre) showmessage(L('model_table_not_exists'));;
		...
		if(isset($_GET['start_time']) && $_GET['start_time']) {
			...
			if(isset($_GET['keyword']) && !empty($_GET['keyword'])) {
				...
				if(isset($_GET['type_name']) && $_GET['type_name'] == 'wirtes'){
			...
			if( isset($_GET['typeid']) && !empty($_GET['typeid']) ){
				$typeid = $_GET['typeid'] > 0 ? intval($_GET['typeid']) : 0;
			...
			for($i=1;$i<=4;$i++) {
				...
				$current = $steps==$i ? 'class=on' : '';
				$r = $this->db->get_one(array('catid'=>$catid,'status'=>$i));
				$newimg = $r ? '<img src="'.IMG_PATH.'icon/new.png" style="padding-bottom:2px" onclick="window.location.href=\'?m=content&c=content&a=init2&modelid='.$modelid.'&steps='.$i.'&pc_hash='.$pc_hash.'\'">' : '';
				$workflow_menu .= '<a href="?m=content&c=content&a=init2&modelid='.$modelid.'&pc_hash='.$pc_hash.'&steps='.$i.'" '.$current.' ><em>'.L('workflow_'.$i).$newimg.'</em></a><span>|</span>';
			}
			if($workflow_menu) {
				$current = isset($_GET['reject']) ? 'class=on' : '';
				$workflow_menu .= '<a href="?m=content&c=content&a=init2&modelid='.$modelid.'&pc_hash='.$pc_hash.'&reject=1" '.$current.' ><em>'.L('reject').'</em></a><span>|</span>';
			}
			$model_fields = getcache('model_field_'.$modelid, 'model');
			$setting = string2array($model_fields['thumb']['setting']);
			$args = '1,'.$setting['upload_allowext'].','.$setting['isselectimage'].','.$setting['images_width'].','.$setting['images_height'].','.$setting['watermark'];
			...
			//更改内容状态
				...
				if ($status==99) {
					...
					if (isset($_POST['ids']) && !empty($_POST['ids'])) {
						foreach ($_POST['ids'] as $id) {
							if( !isset($_POST['token']) && $_POST['token'] != "TOKEN_AJAX_TEMPLATE" && $this->siteid == 3 ){
								$arr_ids 	= $_POST['ids'];
