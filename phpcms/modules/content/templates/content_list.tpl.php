...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?m=content&c=content&a=add&menuid=&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>&pc_hash=<?php echo $_SESSION['pc_hash'];?>','')><em><?php echo L('add_content');?></em></a>　
...
<div class="table-list">
    <table width="100%">
        <thead>
            ...
            <th width="40" onclick="type_sort(this)" alt='id' style="font-weight:blod;color:red;">ID</th>
...
<tbody>
	...
	?>
		...
		<td align='center' onclick="change_keywords(this)" title="title" ><?php echo $r['title'];?></td>
		<td>
			<span onclick="change_keywords(this)" title="seotitle"><?php echo $r['seotitle'];?></span>
		<?php
		if($status==99) {
			if($r['islink']) {
...
var val;
function change_keywords(e)
{
	if($('.change_keywords').length > 0){
		return false;
	}
	val = $(e).html();
	var name = $(e).attr('title');
	var input = "<input type='text' name='"+name+"' class='change_keywords' style='width:300px;' onblur='blur_keywords(this)' value='"+val+"'>";
	$(e).html(input);
	$('.change_keywords').focus();
}
...
function blur_keywords(e)
{
	var value = $(e).val();
	if(val != value){
		var id = $(e).parent().parent().find('.inputcheckbox').val();
		if(id == undefined){
			var id = $(e).parent().parent().parent().find('.inputcheckbox').val();
		}
		var name = $(e).attr("name");
		$.post("index.php?m=content&c=index&a=keywords_edit&modelid=<?php echo $modelid;?>",{name:name,value:value,id:id},function(){});
	}
	$(e).parent().html(value);
}
//-->
